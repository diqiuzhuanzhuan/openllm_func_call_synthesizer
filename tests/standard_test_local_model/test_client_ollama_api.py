...
# client = Client(host="http://192.168.111.11:11434")
client = Client(host="http://192.168.111.3:11434")
# model_name = "qwen3:1.7b"
model_name = "function_call_rl_0104-q4_K_M"
...
def get_client_ollama_response_chat(system_prompt, text):
    ...
    tool_call_matches = re.findall(r"<tool_call>\s*(.+?)\s*</tool_call>", content, flags=re.DOTALL)
    if tool_call_matches:
        tool_calls_list = []
        for block in tool_call_matches:
            # 支持多行多个json
    ...
    # 无论是否有 tool_call，都统一过滤掉 <think> 部分
