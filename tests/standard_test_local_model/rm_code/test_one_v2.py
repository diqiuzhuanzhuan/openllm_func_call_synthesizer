...
def get_response_vllm(system_prompt, user_input, api_url=URL, model_name="qwen3_1.7b_mix", temperature=0.01, top_p=0.1, max_tokens=500):
    """
...
def get_one(x):
    try:
       rs = get_response_vllm(system_prompt, x)
       return parse_fuction_call_response(rs)
    except Exception as e:
        print(e)
        return None

df['llm_response'] = df['input'].apply(lambda x: get_one(x))
