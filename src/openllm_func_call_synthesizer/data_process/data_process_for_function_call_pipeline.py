...
##从表格数据生成llama_factory训练的数据集，并且切分。表格中必须有key_input_column，key_output_column
## 数据会去重


key_input_column = "query"
key_output_column = "ground truth" #function_call
...
if __name__ == "__main__":
    ...
    # )
    # 读取合并去重后的数据
    df = pd.read_excel('/data/work/CHenXuFei/openllm_func_call_synthesizer/data/0128badcase/0128_RL_lora_sft_badcase.xlsx')
    ...
    df.to_excel(
        "/data/work/CHenXuFei/openllm_func_call_synthesizer/data/0128badcase/test_messages.xlsx",
        index=False
    )
    print(f"已保存，共 {len(df)} 条数据")

    # --- Dataset Split and Export ---
    print("\n--- Starting Dataset Split ---")
    import os
    
    # Configuration
    TEST_RATIO = 0.1                   # Test set ratio (Random)
    VAL_RATIO = 0.1       # Validation ratio 

    output_dir = "/data/work/CHenXuFei/openllm_func_call_synthesizer/data/test"

    # 1. Split Set (Random, strictly random as requested)
    # Reset index to ensure basic 0..N range for clean slicing initially
    df_shuffled = df.sample(frac=1, random_state=42).reset_index(drop=True)
    num_test = int(len(df_shuffled) * TEST_RATIO)
    
    df_test = df_shuffled.iloc[:num_test]
    df_val = df_shuffled.iloc[num_test:num_test + int(len(df_shuffled) * VAL_RATIO)]
    df_train = df_shuffled.iloc[num_test + int(len(df_shuffled) * VAL_RATIO):]
    ...
    def save_json(dataframe, filename):
        ...
        data_list = dataframe["lora_input"].tolist()
    ...
    save_json(df_train, "mcp_train_fc.json")
    save_json(df_val, "mcp_dev_fc.json")
    save_json(df_test, "mcp_test_fc.json")
