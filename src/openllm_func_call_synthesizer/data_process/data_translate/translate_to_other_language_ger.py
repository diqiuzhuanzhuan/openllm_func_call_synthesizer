...
system_prompt_ger = """
你是一个智能翻译助手，你将协助用户将输入内容翻译成德语，只输出翻译本身，不要任何解释说明。
注意: 1.中文人名替换成德国名字， 明星替换成欧美明星的名字
      2.中文电影名替换成德语或英语电影名，如 琅琊榜--->Good Will Hunting  或 Das Leben der Anderen 或 Heidi
...
翻译输出：
{
"input": "Gibt es in letzter Zeit neue Filme zu sehen?",
"output": "{"intent": "video_search_control", "slots": {"title": "neue Filme", "type": "movie"}}"
}
...
input_path = "./data_process/new_mcp_datas/mcp_zh_data_1014_v2.xlsx"
output_path = "./data_process/new_mcp_datas/mcp_zh_data_1014_v2_ger.xlsx"
output_path_check = "./data_process/new_mcp_datas/mcp_zh_data_1014_v2_ger_check.xlsx"
...
for _idx, line in df.iterrows():
    ...
    result = get_llm_response(system_prompt_ger, user_input)
    print(result)  # 观察模型输出
...
def contains_german(text):
    # 判断字符串是否包含德语字符（重点判断含有特殊德文字母 öäüß 和常用德语字母组合）
    if not isinstance(text, str):
        return False
    # 检查特殊德语字符
    if re.search(r"[äöüÄÖÜß]", text):
        return True
    # 可以扩展关键德语词检测，如冠词、常用连词、名词大写。这里只检测特殊字符
    return False
...
def slots_german_in_intent(output, intent_text):
    try:
...
df["slots_in_intent"] = df.apply(lambda x: slots_german_in_intent(x["output"], x["input"]), axis=1)

df["slots_in_intent"].value_counts()
